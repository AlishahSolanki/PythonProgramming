...
Grading Rubric: 25 points total
 0.5 pts: import sys
   ...
   1 pt : -- assign parameter to number instance variable
   2 pts: -- compute internal angle (1 pt), assign to angles instance variable
 ...
 0.5 pts: -- else:
   2 pts: -- -- instantiate Polygon object (1 pt) with argument (0.5 pts), assignment (0.5 pts)
...
class Polygon:
    def __init__(self, numSides):
        self.number = numSides
        self.angles = (self.number - 2) * 180 / self.number
    def __str__(self):
        return 'sides=' + str(self.number) + ', internal angles=' + str(self.angles)
...
while i < len(sys.argv):
    try:
        ...
        my_poly = Polygon(int(sys.argv[i]))
        print(my_poly)
    finally:
        i += 1
